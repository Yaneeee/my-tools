...
gap = st.sidebar.number_input('空隙经验系数:', min_value=1.0, max_value=1.1, value=1.03, step=0.01)
...
if choice=='卷径计算':
    ...
    length = st.number_input('长度(米/M):', value=2000, step=10)
    if length>0:
        D = calculate_diameter(length, thickness, d0, gap)
    ...
    D = st.number_input('卷径(厘米/cm):', min_value=d0 * 2.54 + 2.8, value=55.0, step=0.1)
    if D>d0:
        length = calculate_length(D, thickness, d0, gap)
